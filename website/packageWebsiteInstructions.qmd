...
5. Locally, run `quarto publish gh-pages website`, and then `Y` to the option that arises. This will set-up and publish the first version of the website to gh-pages branch.
...
It's a bit strange, but to properlly set up a GHA using standard pre-existing actions (the easiest thing to do), you first have to do step (5) from above. This is needed for the action to know where / how to publish in the future. I haven't found a good way around this yet, but in theory you could modify the action so that you don't need the initial publish.
